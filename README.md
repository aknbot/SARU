...
会社で受ける検定試験（ビジネス会計検定 3級 / 2級、銀行業務検定 財務3級）の学習アプリ。静的サイト（GitHub Pages）＋ Supabase（Google ログイン・進捗同期）で動く。

- 要点ノート（赤シート）、一問一答、資料問題、本番形式の模擬試験、試験日から逆算した学習予定
- Google でログインして使う（`config.js` の `requireLogin: true`）。進捗はキー単位でマージ同期され、複数端末で使える
- 試験回（日程・締切・受験料）は `exams.js` のマスタから自動反映。回が変わっても教材は書き換えない。CBT 通年実施の試験（財務3級）は `CBT` に実施期間を書き、学習予定は受験予定日から逆算する
- 「ビジネス会計検定試験」は大阪商工会議所、「銀行業務検定試験」は銀行業務検定協会の登録商標。本サービスはいずれの団体とも無関係の学習教材
...
exams.js              試験回マスタ（試験日・申込締切・受験票・合格発表・集合時刻・受験料・合格率・CBT 実施期間）
courses.js            コース一覧
courses/<id>/         各コースの中身
  course.js             ステップ構成（相対日数）・テーマ色・使い方・当日の動き方・模試の科目構成（parts）・ノート参照表（refs）
...
- 問題の解説に出る「ノートで確認」リンクは `course.js` の `refs`（`{re, h}` の配列）で決まる。問題文（当たらなければ問題文＋解説）に `re` が当たった最初の行の見出し `h`（ノートの `<h3>` の先頭一致）へ飛ぶ。`weak:1` の行は問題文だけの判定では飛ばされる。どの行にも当たらなければ、そのセットを扱うステップの `read` 章へ。新しい見出しを足したら `refs` にも 1 行足す
