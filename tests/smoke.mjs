...
// 3. 進捗の単体ロジック（マージ・移行・予定生成）
{
  ...
  check('legacy local progress migrated to user namespace', mig.legacyGone && mig.sched && mig.sched['1'] === true && mig.best && mig.best['1'] && mig.best['1'].p === 60, JSON.stringify(mig));
  ...
  const unit = await p.evaluate(() => {
    const a = { v: 2, sched: { 1: true }, wrong: { 'bk3-aaaaaaaa': 10 }, cleared: {}, best: { 1: { p: 50, c: 5, t: 10 } }, applied: false, updatedAt: 100 };
    const b = { v: 2, sched: { 2: true }, wrong: {}, cleared: { 'bk3-aaaaaaaa': 20 }, best: { 1: { p: 80, c: 8, t: 10 }, 2: { p: 40, c: 4, t: 10 } }, applied: true, updatedAt: 90 };
    ...
    return { m, plan: plan.steps.map(s => [s.s, s.e]), compressed: plan.compressed, bad };
  });
  check('merge keeps both sched keys', unit.m.sched['1'] === true && unit.m.sched['2'] === true);
  check('merge: later correct answer removes wrong', !unit.m.wrong['bk3-aaaaaaaa'] && unit.m.cleared['bk3-aaaaaaaa'] === 20);
  check('merge: best takes max per set', unit.m.best['1'].p === 80 && unit.m.best['2'].p === 40 && unit.m.applied === true);
